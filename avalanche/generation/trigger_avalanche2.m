function Y_t = trigger_avalanche2(A, B, u_t, max_duration)
%trigger_avalanche - partially vectorized
...
for t = 1 : max_duration
    for j = 1 : N % firing Y, neuron i -> neuron j
        spikes_u = sum(binornd(u_t(j,t), B(j)));
        spikes_Y_j = 0;
        if t > 1
            spikes_Y_j = spikes_Y_j + sum(binornd(Y_t(:,t-1), A(:,j)));
        end
